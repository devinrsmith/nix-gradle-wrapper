...
- **`gradle-wrapper.nix`** — the entire implementation, a single `{ pkgs, wrapperPropertiesFile,
  name, perWorkerMemBytes ?, daemonMemBytes ?, otherMemBytes ? }: { ... }` function. Internally it
  does two independent things that are both worth understanding before editing either:
  ...
  2. **Toolchain isolation** (`isolatedHomeHook`): redirects `GRADLE_USER_HOME` to an isolated,
     ...
     divided by `perWorkerMemBytes`; best-effort across Linux `/proc/meminfo` and macOS `sysctl
...
- **`tests/unit.nix`** — pure eval-level tests over the parsing/unescaping/path-derivation logic
  (colon-unescaping, `-all`/`-bin` suffix stripping, mirror URLs with ports, nested paths). Runs
  with fake, never-fetched URLs and a placeholder sha256, since `pkgs.fetchurl` only touches the
...
- **`tests/integration.nix`** — the one layer that does a real build: fetches
  ...
  `GRADLE_USER_HOME`, `gradle.properties` contents, and wrapper-cache directory layout are exactly
  what a real `./gradlew` would look for. Notably references the fixture zip via `"${self}/..."`
  (the whole flake source, already one store copy) rather than a fresh `${./relative/path}`
  interpolation — the latter would re-add the file to the store as `<hash>-<basename>`, corrupting
